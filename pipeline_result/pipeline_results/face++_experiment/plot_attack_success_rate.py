...
def plot_median_loss_curve(all_loss_dfs, model_name="face_analysis_cli"):
    ...
    ax.plot(interp_queries, median_loss, label='Median Loss')
    ax.fill_between(interp_queries, q1_loss, q3_loss, alpha=0.2, label='Interquartile Range (IQR)')
    
    ax.set_title(f'{model_name} Median Attack Loss vs. Number of Queries', fontsize=16)
    ax.set_xlabel('Number of Queries', fontsize=12)
    ax.set_ylabel('Loss', fontsize=12)
    # plt.grid(True)
    ax.legend()
    ax.set_xlim(left=0, right=max_queries_limit)
    ax.set_ylim(bottom=29)

    output_filename = f'median_loss_curve_{model_name}.pdf'
...
def main():
    ...
    ax.plot(query_bins, success_rate_axis, linestyle='-', label='Face++ Recognition Pipeline')
    
    ax.set_title('Attack Success Rate vs. Number of Queries', fontsize=16)
    ax.set_xlabel('Number of Queries', fontsize=12)
    ax.set_ylabel('Attack Success Rate', fontsize=12)
    # plt.grid(True, which='both', linestyle='--', linewidth=0.5)
    ax.legend()
    
    ax.set_xlim(0, max_queries_limit)
    ax.set_ylim(0, 0.6)
    
    # Set y-axis ticks to be percentages
    ax.yaxis.set_major_formatter(plt.FuncFormatter('{:.0%}'.format))
    
    output_filename = 'attack_success_rate_face++.pdf'
    fig.savefig(output_filename, format="pdf", bbox_inches="tight")
