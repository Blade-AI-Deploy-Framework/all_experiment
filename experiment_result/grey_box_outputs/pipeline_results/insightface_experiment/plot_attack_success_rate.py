...
def plot_success_rate_vs_queries(all_results):
    """
    Plots the success rate vs. number of queries for all models, one plot per model.
    """
    ...
    for model_name, data in all_results.items():
        ...
        plt.plot(query_bins, success_rate, linestyle='-', label="insightface recognition pipeline")

        plt.title(f'Attack Success Rate vs. Number of Queries')
        ...
        output_filename = f'attack_success_rate_{model_name}.png'
...
def plot_average_loss_curve(all_results):
    ...
    for model_name, data in all_results.items():
        ...
        # Plot the mean loss
        plt.plot(interp_queries, mean_loss, label='Average Loss')
        
        # Plot the shaded standard deviation area
        plt.fill_between(interp_queries, mean_loss - std_loss, mean_loss + std_loss, alpha=0.2, label='Standard Deviation')
        
        plt.title(f'Average Attack Loss vs. Number of Queries')
        plt.xlabel('Number of Queries')
        plt.ylabel('Loss')
        
        # Add vertical lines as requested
        plt.axvline(x=1000, color='darkgoldenrod', linestyle='--')
        plt.axvline(x=11000, color='red', linestyle='--')
        plt.axvline(x=45000, color='green', linestyle='--')
        ...
        output_filename = f'average_loss_curve_{model_name}.png'
        plt.savefig(output_filename)
        print(f"Plot saved to: {output_filename}")
        plt.close()
...
def plot_loss_boxplot_vs_queries(all_results):
    ...
    for model_name, data in all_results.items():
        ...
        plt.title(f'{model_name} Loss Distribution vs. Number of Queries')
